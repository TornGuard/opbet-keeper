...
export async function upsertBet(params) {
    const { betId, betType, param1, param2, amount, endBlock, contractAddress } = params;
    await pool.query(`INSERT INTO bets (bet_id, bet_type, param1, param2, amount, end_block, contract_address)
 ...
 */
export async function registerBetOwner(params) {
    const { betId, wallet, tokenSymbol, contractAddress } = params;
    await pool.query(`INSERT INTO bets (bet_id, wallet, token_symbol, contract_address)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (bet_id) DO UPDATE
           SET wallet           = EXCLUDED.wallet,
               token_symbol     = COALESCE(EXCLUDED.token_symbol, bets.token_symbol),
               contract_address = COALESCE(EXCLUDED.contract_address, bets.contract_address)`, [betId, wallet.toLowerCase(), tokenSymbol ?? null, contractAddress ?? null]);
}
...
export async function getBetsByWallet(wallet, contractAddress) {
    ...
    const result = await pool.query(`SELECT bet_id, bet_type, param1, param2, amount, end_block, status, won, payout, wallet, token_symbol, placed_at, resolved_at, resolve_tx
         FROM bets
         WHERE wallet = $1${contractFilter}
         ORDER BY bet_id DESC`, params);
    return result.rows;
}
...
export async function getRecentBets(contractAddress, limit = 20) {
    ...
    const result = await pool.query(`SELECT bet_id, bet_type, param1, param2, amount, end_block, status, won, payout, wallet, token_symbol, placed_at, resolved_at
         FROM bets
...
export async function getBetWithWallet(betId) {
    const result = await pool.query(`SELECT wallet, param1, param2, bet_type, token_symbol FROM bets WHERE bet_id = $1`, [betId]);
    return result.rows[0] ?? null;
}
/** Count consecutive wins for a wallet (streak detection). */
export async function getConsecutiveWins(wallet) {
    const result = await pool.query(`SELECT won FROM bets
         WHERE wallet = $1 AND won IS NOT NULL
         ORDER BY bet_id DESC
         LIMIT 10`, [wallet.toLowerCase()]);
    let streak = 0;
...
export async function getAllBets() {
    const result = await pool.query(`SELECT bet_id, bet_type, param1, param2, amount, end_block, status, won, payout, wallet, placed_at, resolved_at, resolve_tx
         FROM bets ORDER BY bet_id DESC`);
    return result.rows;
}
/** Mark a bet as resolved with its outcome. */
export async function markBetResolved(params) {
    const { betId, won, payout, txId } = params;
    await pool.query(`UPDATE bets
         SET status = 1, won = $2, payout = $3, resolved_at = NOW(), resolve_tx = $4
         WHERE bet_id = $1`, [betId, Boolean(won), payout !== null ? payout.toString() : null, txId ?? null]);
}
/**
 ...
 */
export async function upsertBetOwner(params) {
    const { betId, ownerHex } = params;
    await pool.query(`UPDATE bets SET owner_hex = $1 WHERE bet_id = $2 AND owner_hex IS NULL`, [ownerHex, betId]);
    await pool.query(`INSERT INTO bettors (owner_hex, first_bet, last_bet, bet_count)
         VALUES ($1, $2, $2, 1)
         ON CONFLICT (owner_hex) DO UPDATE
           SET last_bet   = GREATEST(bettors.last_bet, EXCLUDED.last_bet),
               bet_count  = bettors.bet_count + 1,
               updated_at = NOW()`, [ownerHex, betId]);
}
/** Link a p2tr wallet address to an on-chain owner_hex. */
export async function linkBettorWallet(params) {
    const { ownerHex, wallet } = params;
    await pool.query(`INSERT INTO bettors (owner_hex, wallet, first_bet, last_bet, bet_count)
         VALUES ($1, $2, 0, 0, 0)
         ON CONFLICT (owner_hex) DO UPDATE
           SET wallet = COALESCE(EXCLUDED.wallet, bettors.wallet)`, [ownerHex, wallet.toLowerCase()]);
}
/** Return all bettors for airdrop snapshot. */
export async function getAllBettors() {
    const result = await pool.query(`SELECT owner_hex, wallet, first_bet, last_bet, bet_count, created_at
         FROM bettors
         ORDER BY first_bet ASC`);
    return result.rows;
}
/** Record a successful oracle feed submission (skip if already stored). */
export async function upsertOracleFeed(params) {
    const { blockHeight, medianFeeScaled, mempoolCount, txId } = params;
    await pool.query(`INSERT INTO oracle_feeds (block_height, median_fee_scaled, mempool_count, tx_id)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (block_height) DO NOTHING`, [blockHeight, medianFeeScaled, mempoolCount, txId ?? null]);
}
/** Return recent oracle feed rows (last N by block height). */
export async function getRecentFeeds(limit = 50) {
    const result = await pool.query(`SELECT block_height, median_fee_scaled, mempool_count, tx_id, submitted_at
         FROM oracle_feeds ORDER BY block_height DESC LIMIT $1`, [limit]);
    return result.rows;
}
