...
async function getStats(oracle: OracleFeeder, resolver: BetResolver) {
    const [betsResult, feedsResult, bettorsResult, activeBetsResult, recentBets, recentFeeds, feeStats] =
        await Promise.all([
            pool.query<{ count: string }>('SELECT COUNT(*) FROM bets'),
            pool.query<{ count: string }>('SELECT COUNT(*) FROM oracle_feeds'),
            pool.query<{ count: string }>('SELECT COUNT(*) FROM bettors'),
            pool.query<{ count: string }>('SELECT COUNT(*) FROM bets WHERE status = 0'),
...
function sparkline(feeds: { median_fee_scaled: number; submitted_at: unknown }[]): string {
...
function renderHTML(stats: Record<string, any>): string {
    const fix2    = (v: unknown) => v != null ? Number(v).toFixed(2) : '—';
    const shortTx = (tx: string | null) => tx
        ? `<a href="https://opscan.org/transactions/${tx}?network=op_testnet" target="_blank" title="${tx}">${tx.slice(0, 8)}…${tx.slice(-6)}</a>`
        : '—';
    const fmtTime = (d: unknown) => d ? new Date(d as string).toLocaleTimeString('en-GB', { hour12: false }) : '—';
    const fmtDate = (d: unknown) => d ? new Date(d as string).toISOString().replace('T', ' ').slice(0, 16) : '—';
    const shortWallet = (w: string | null) => w ? `<span title="${w}">${w.slice(0, 6)}…${w.slice(-4)}</span>` : '<span style="color:#333">?</span>';

    const fs     = stats['feeStats'] ?? {};
    const behind = stats['lastOPNetBlock'] - stats['lastSubmittedBlock'];
    const behindColor = behind > 5 ? 'orange' : 'green';
    const neededHtml  = (stats['neededBlocks'] as number[]).length
        ? (stats['neededBlocks'] as number[]).sort((a, b) => a - b).map(b => `<span class="pill">${b}</span>`).join(' ')
        : '<span class="dim">none</span>';

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const txLog: { time: unknown; type: string; label: string; detail: string; tx: string }[] = [];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    for (const f of stats['recentFeeds'] as any[]) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    for (const b of stats['recentBets'] as any[]) {
    ...
    txLog.sort((a, b) => new Date(b.time as string).getTime() - new Date(a.time as string).getTime());
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const betsRows = (stats['recentBets'] as any[]).slice(0, 10).map(b => {
...
<div class="stats">
  <div class="stat"><div class="lbl">Uptime</div><div class="val" style="font-size:14px">${formatUptime(stats['uptime'] as number)}</div></div>
  <div class="stat"><div class="lbl">OPNet Block</div><div class="val" style="color:#f7931a">${(stats['lastOPNetBlock'] as number).toLocaleString()}</div></div>
  ...
  <div class="stat"><div class="lbl">Mempool</div><div class="val" style="font-size:14px">${(stats['latestMempoolCount'] as number).toLocaleString()}</div><div class="sub">txs</div></div>
...
<div class="spark"><h3 style="font-size:10px;color:#444;text-transform:uppercase;margin-bottom:8px">BTC Median Fee — last ${(stats['recentFeeds'] as unknown[]).length} submissions</h3>
  ${sparkline(stats['recentFeeds'] as { median_fee_scaled: number; submitted_at: unknown }[])}
</div>
...
export function startHealthServer(oracle: OracleFeeder, resolver: BetResolver): void {
    ...
    const server = http.createServer(async (req, res) => {
        ...
        try {
            if (req.method === 'POST' && url.pathname === '/api/bets') {
                ...
                const betId = body['betId'], wallet = body['wallet'] as string | undefined;
                if (!betId || !wallet) {
                    res.writeHead(400, { ...CORS_HEADERS, 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ error: 'betId and wallet are required' }));
                    return;
                }
                await registerBetOwner({ betId: Number(betId), wallet, tokenSymbol: body['tokenSymbol'] as string | null, contractAddress: body['contractAddress'] as string | null });
                if (body['betType'] !== undefined) {
                    notifyEntry({
                        betId: Number(betId), wallet, txId: body['txId'] as string ?? null,
                        betType: Number(body['betType']), param1: Number(body['param1'] ?? 0),
                        param2: Number(body['param2'] ?? 0), amount: body['amount'] as bigint ?? null,
                        endBlock: body['endBlock'] as number ?? null, tokenSymbol: body['tokenSymbol'] as string ?? null,
                    }).catch((err: Error) => console.warn('[Telegram] Entry notify error:', err.message));
                }
            ...
            if (req.method === 'GET' && url.pathname === '/api/leaderboard') {
                ...
                const result  = await pool.query(
                    `SELECT wallet, COUNT(*) FILTER (WHERE won = true)::int AS wins,
                     COUNT(*) FILTER (WHERE won = false)::int AS losses,
                     COALESCE(SUM(payout::numeric) FILTER (WHERE won = true), 0)::text AS total_payout
                     FROM bets WHERE wallet IS NOT NULL${filter} GROUP BY wallet ORDER BY total_payout::numeric DESC LIMIT 10`,
                    params,
                );
                res.writeHead(200, { ...CORS_HEADERS, 'Content-Type': 'application/json' });
            ...
            if (req.method === 'GET' && url.pathname === '/api/bettors') {
                ...
                if (format === 'csv') {
                    ...
                    for (const b of bettors) rows.push(`${b.owner_hex},${b.wallet ?? ''},${b.first_bet},${b.last_bet},${b.bet_count}`);
